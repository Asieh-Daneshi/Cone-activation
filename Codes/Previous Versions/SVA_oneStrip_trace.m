...
tic
close all
clearvars -except alllocs_OneStrip crossFlagOne crossFlagStartOne crossFlagEndOne videoName videoNames PathName WCentroid20 boxposition conelocs sumNorm_Jenny alllocs_OneStrip_backup matName1
clc
warning('off','all')
%%
...
ZF=1;
cropSize=710*ZF+1;
FullField=600;  % In pixel of imaging grid
...
for a1=1:fNom
        ...
        if ~isempty(find(CurrentFrame))&& ismember(a1,crossFlagOne) && alllocs_OneStrip(a1,2)*ZF-size(myStim,1)/2*ZF>boxposition(1) && alllocs_OneStrip(a1,2)*ZF+size(myStim,1)/2*ZF<boxposition(3) && alllocs_OneStrip(a1,1)*ZF-size(myStim,2)/2*ZF>boxposition(2) && alllocs_OneStrip(a1,1)*ZF+size(myStim,2)/2*ZF<boxposition(4)
            a3=a3+1;
            stimIm=zeros(cropSize);
            stimIm(alllocs_OneStrip(a1,2)*ZF-size(myStim,1)/2*ZF:alllocs_OneStrip(a1,2)*ZF+size(myStim,1)/2*ZF,alllocs_OneStrip(a1,1)*ZF-size(myStim,2)/2*ZF:alllocs_OneStrip(a1,1)*ZF+size(myStim,2)/2*ZF)=imresize(myStim(:,:,a3),[size(myStim,1)*ZF+1,size(myStim,2)*ZF+1]);  % For square stimulus

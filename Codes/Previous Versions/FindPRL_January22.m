...
[videoName,PathName]=uigetfile('*_stabilized.avi','MultiSelect', 'off');
...
alllocs_OneStrip=NaN(fNom,2);
for a1=1:fNom
    shiftVar(a1,1)=a1;
    [x,y,crossFlag]=Cross_oneFrame_November4(myVideo,10,a1);
    if crossFlag==1
        shiftVar(a1,2:3)=[x,y];
        alllocs_OneStrip(a1,2)=shiftVar(a1,3);
        alllocs_OneStrip(a1,1)=shiftVar(a1,2);
    end
end
%% ========================================================================
% finding cross locations in the remaining frames -------------------------
x=nanmedian(alllocs_OneStrip(:,1));
y=nanmedian(alllocs_OneStrip(:,2));
crossFlagOne=find(~isnan(alllocs_OneStrip(:,1))==1);
